...
#def parameters_callback(params):
 ...
 #   global JETBOT_MAX_ANG_VEL
   ...
   #         self.max_angular_vel = param.value
    #    else:
    #        raise ValueError('unknown parameter'.format(param.name))
...
def main():
    ...
    rospy.init_node('teleop_keyboard')  #, namespace='jetbot'
    pub = rospy.Publisher('/jetbot_velocity_controller/cmd_vel',Twist, queue_size=10)
    #key_pub = node.create_publisher(String, 'keys', qos)
    key_pub = rospy.Publisher('keys', String, queue_size=10)
    
    rospy.set_param('max_linear_vel', JETBOT_MAX_LIN_VEL)
    rospy.set_param('max_angular_vel', JETBOT_MAX_ANG_VEL)
    
    JETBOT_MAX_LIN_VEL = rospy.get_param('max_linear_vel')
    JETBOT_MAX_ANG_VEL = rospy.get_param('max_angular_vel')
